package com.example.代码优化.设计模式之美.a_29代码的可测试性.first;
...
public class Transaction {
	...
	public boolean execute() throws InvalidTransactionException {
		...
		try{
			isLocked = RedisDistributedLock.getInstance().lockTransction(id);
			if(!isLocked) {
			...
			if(isLocked) {
				RedisDistributedLock.getInstance().unlockTransction(id);
			}
		}
	}
}
